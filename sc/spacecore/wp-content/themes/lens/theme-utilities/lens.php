	...
	static function display_filter_box( $taxonomy_name ){
        ...
        if(!empty($filter_categories) && !is_wp_error($filter_categories)) { ?>
            <div class="mosaic__filter sticky-button">
                <div class="filter__btn sticky-button__btn btn btn--large"><i class="pixcode  pixcode--icon  icon-e-up-circled"></i><span><?php _e('Filter', wpgrade::textdomain()); ?></span>
                    <ul>
                        ...
                        <?php } ?>
                        <li class="mosaic__filter-item sticky-button-item" data-filter="all"><?php _e('View All', wpgrade::textdomain()); ?></li>
                    </ul>
                </div>
            </div>
        <?php
