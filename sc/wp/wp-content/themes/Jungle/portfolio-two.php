...
<?php
	$loop = new WP_Query(array('post_type' => 'portfolio', 'posts_per_page' => ot_get_option('portfolio_items_number', '8', '', '', ''), 'paged' => $paged));
	$count = 1;
?>
...
<div class="row">
	...
	<div class="title">
		...
		foreach($categories as $category) { ?>
			<li>
				<a href="#" data-filter=".<?php echo $category->slug; ?>"><?php echo $category->cat_name; ?></a>
			</li>
		<?php } ?>
		</ul>
		<?php } else { } ?>
...
<div class="row portfolio-col-2 spacing-bottom">
		...
		<div class="image">
			...
			<nav class="hover-card">
				<ul class="thumb-options">
					...
					<li class="thumb-zoom">
						<a <?php if($video == true) { ?> href="<?php echo $video;?>" <?php } else { ?> href="<?php echo $lightbox_img;?>" <?php }?> rel="prettyPhoto">Zoom</a>
					</li>
				</ul>
			</nav>
			<?php } else { } ?>
			<a href="<?php the_permalink(); ?>"><?php echo the_post_thumbnail('portfolio_one_two', array('class' => 'scale-with-grid' )); ?></a>
	    </div><!-- End of image -->
